document.addEventListener("DOMContentLoaded", () => {
  ...
  // ============================
  // JOIN BUTTON EFFECT (SOUND REMOVED)
  // ============================
...
🔹 Consult a trainer for a guided plan
  ...
  (function branchesModal() {
    ...
    const openMap = (title, mapUrl) => {
      ...
      if (mapTitle) mapTitle.textContent = title || "Branch Location";
      mapFrame.src = mapUrl || "";
    ...
    document.querySelectorAll(".branch-card").forEach((card) => {
      ...
      if (viewBtn)
        viewBtn.addEventListener("click", () =>
          openLightbox(imgSrc, branchName),
        );
      if (imgEl) {
  ...
  (function barberLightbox() {
    ...
    if (barberImage) {
      ...
      barberImage.addEventListener("click", () => {
        if (!lb || !lbImg) return;
        lbImg.src = barberImage.src;
        lbImg.alt = barberImage.alt;
        lb.style.display = "flex";
      });
    }
  ...
  (function fitBot() {
    ...
    const MODE_KEY = "alpha_fitbot_mode_v1";
    const MEMORY_KEY = "alpha_fitbot_memory_v1";
    const TRACK_KEY = "alpha_fitbot_tracker_v1";
    ...
    // ============================
    // 1) Dynamic Quick Replies  ✅ MOVED ABOVE intro() CALL
    // ============================
    const quickSets = {
      default: [
    ...
    // show default initially (safe now)
    renderQuickReplies("default");
    ...
    // ✅ restore messages AFTER quickSets/renderQuickReplies/intro exist
    ...
    // ============================
    // Everything below is your original chatbot logic (unchanged)
    // ============================
    ...
    function send(text) {
      ...
      if (timerCmd) {
        bot(
          tagPrefix +
            `Starting ${timerCmd.kind} for **${timerCmd.seconds}s** ⏱️`,
        );
        startTimer(
          timerCmd.seconds,
          timerCmd.kind === "rest" ? "Rest" : "Timer",
        );
        renderQuickReplies("workout");
        return;
      }
      ...
      if (logCmd) {
        ...
        if (!ok) {
          bot(
            tagPrefix +
              "Please enter a valid number. Example: `log steps 8000`",
          );
          return;
        }
        bot(
    ...
    function handleSetup(text) {
      ...
      if (mode === "setup_minutes") {
        const n = parseInt(lower, 10);
        profile.minutes = Number.isFinite(n)
          ? Math.min(90, Math.max(20, n))
          : 45;
        saveProfile();
        mode = "normal";
    ...
    function makePlan(
      ...
      if (goal === "muscle") {
        return [
          ...
          d >= 4
            ? "Day 4: Upper — DB Bench 3x8–12, Row 3x8–12, Arms 3 sets"
            : "",
          d >= 5
            ? "Day 5: Lower — Hack/Front Squat 3x6–10, Ham Curl 4x10–15, Core"
            : "",
          d >= 6
            ? "Day 6: Weak points — rear delts/arms/calves (pump work)"
            : "",
  ...
  // ============================
  // FREE PASS POPUP (Homepage only)
  // ============================
  (function freePassPopup() {
    const path = (window.location.pathname || "").toLowerCase();
    const isHome =
      path === "/" || path.endsWith("/index.html") || path.includes("index");
    if (!isHome) return;
    ...
    function showPopup() {
      ...
      wrap.innerHTML = `
        ...
        <div class="fp-card" role="dialog" aria-modal="true" aria-label="Free pass offer">
          ...
          <div class="fp-sub">
            🎟️ Free pass is limited daily. <b>Subject to confirmation.</b><br>
            Submit now and we’ll confirm availability within <b>24 hours</b>.
          </div>

          <div class="fp-bullets">
            <div>✅ <span><b>No login</b> • takes 30 seconds</span></div>
            <div>📍 <span><b>Choose a branch</b> • we’ll confirm your slot</span></div>
            <div>⚡ <span><b>Fast reply</b> • within 24 hours</span></div>
          </div>
